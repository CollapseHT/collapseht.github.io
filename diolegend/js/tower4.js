...
function handleButtonClick(event) {
    ...
    if (isgo == 0 && start_x >= 500 && start_x <= 700 && start_y >= 300 && start_y <= 500) {
        ...
        talkimg.addEventListener("load", (e) => {
            talk(100, 200, '結局：帝國皇帝死去使整個帝國滅亡，王子迪歐帶手下回國，在民眾的歡呼裡迪歐繼位為國王，王國又回到和平裡，以往不同的是王國領土比之前大了一點，多出來的土地因為不能耕種迪歐下令建立前線保壘，想必怕第二個帝國入侵吧。', '感謝大家的遊玩，雖然製作過程中經歷過了小組衝突，但還是大家齊心協力的把這個小遊戲做了出來，再次感謝大家的參與。');
            //setTimeout(ctx.drawImage(bg, 0, 0), 5000)
            setTimeout(go, 99999999);
        });
...
function go() {
    ...
    MS[0] = new obj(145, 394, 0, 300, 380, ['base.png'], ['base.png'], 1000, 1, 10, 0);
    ES[0] = new obj(1060, 410, 0, 300, 380, ['enemybase.png'], ['enemybase.png'], 1000, 1, 10, 1);
    MS[1] = new boss(0, 400, 40, 120, 120, ['dio_walk1.png', 'dio_walk2.png'], ['dio_atk1.png', 'dio_atk2.png'], 600, 0, 10, 0, 80);
    ES[1] = new boss(1200, 400, 40, 120, 120, ['eh_walk1.png', 'eh_walk2.png'], ['eh_atk1.png', 'eh_atk2.png'], 600, 0, 10, 1, 80);
...
function gainmoney() {
    ...
    ctx.font = "40px serif";
    ctx.fillStyle = "black";
    ctx.fillText("Coins: " + money + "/300  " + " || " + "Waves: " + esendcnt, 720, 750);
    ctx.fillText("步兵: 10$   ||   騎兵: 20$   ||   弓兵: 30$", 20, 640);
    ctx.fillText(document.getElementById("status").innerHTML, 720, 660);
}
...
function esends(x) {
    ...
    ES[num] = new obj(1200, Math.floor((Math.random() * 247) + 283), 25, 128, 124, [
        'e_solder_walk1.png', 'e_solder_walk2.png'
    ], ['e_solder_atk1.png', 'e_solder_atk2.png'], 70, 0, 1);
    } else if (echance > 0.33 && echance <= 0.67) {
