...
function handleButtonClick(event) {
    ...
    if (isgo == 0 && start_x >= 500 && start_x <= 700 && start_y >= 300 && start_y <= 500) {
        ...
        talkimg.addEventListener("load", (e) => {
            talk(100, 200, '劇情2：我們守住王國了，我們一定要重整軍隊，雖然帝國軍撤到帝國境內沙漠荒野裡，但是帝國還會再犯，迪歐思索一番決定先下手為強命令眾將們開始反攻打到帝國境內。(本關為沙漠，敵方騎兵出現機率提升，敵方騎兵血量小幅提升，雙方弓兵傷害距離小幅縮短)', '遊玩方法及規則：用滑鼠點擊出兵鈕進行攻打，打爆敵方主堡即可獲勝，反之自己主堡爆了就失敗。');
            ...
            for (var i = 0; i < ES.length; i++) {
                ...
                if (dist2 < DD * DD && dist2 >= LESS) {
                    ...
                    if (ES[i].hp <= 0) {
                        if (i == 0) {
                            clearInterval(t);
                            alert("恭喜第二關過關！\n經歷了重重關卡，迪歐馬上就要到索默帝國的首都了，迪歐能不能扳回一成，取得敵方將領的首級呢？(請點擊第三關按鈕繼續遊戲)");
                            /* 關卡遞增 */
                            if (stage > htmlstage) {
                                stage = stage;
                            } else {
    ...
    this.draw = function (ctx) {
        ...
        if (this.tp == 2 && this.att.length > 0) {
            ...
            for (var ii = 0; ii < this.att.length; ii++) {
                ...
                ctx.lineTo(this.att[0].x, this.att[0].y + 5);
                ctx.stroke();
            }
        }
    }
...
function go() {
    ...
    MS[0] = new obj(145, 394, 0, 300, 380, ['base.png'], ['base.png'], 1500, 1, 10, 0);
    ES[0] = new obj(1060, 410, 0, 300, 380, ['enemybase.png'], ['enemybase.png'], 1500, 1, 10, 1);
    MS[1] = new boss(0, 400, 40, 120, 120, ['dio_walk1.png', 'dio_walk2.png'], ['dio_atk1.png', 'dio_atk2.png'], 800, 0, 10, 0, 80);
    ES[1] = new boss(1200, 400, 40, 120, 120, ['eh_walk1.png', 'eh_walk2.png'], ['eh_atk1.png', 'eh_atk2.png'], 1000, 0, 10, 1, 80);
